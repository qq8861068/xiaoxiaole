...
@ccclass
export default class GuidView9 extends ViewBase {
    ...
    refreshView(isFristRefresh:boolean = false){
        if(UIManager.getInstance().isShow(Define.viewTurntable)==false){
            return;
        }
        let small=this.small=10;
    ...
    update(dt:number){
        if(UIManager.getInstance().isShow(Define.viewTurntable)==false){
            GuidanceMgr.getInstance().closeGuid();
            UIManager.getInstance().hideView(Define.viewGuid9);
        }
    }
}