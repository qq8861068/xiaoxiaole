...
@ccclass
export default class FeedYuView extends ViewBase {
    ...
    onClick(tag:string){
        if(tag == "btn_back"){
            UIManager.getInstance().hideView(Define.viewFeedYu);
            ...
            if(this.consume>UserInfo.userGold){
                ...
                UIManager.getInstance().hideView(Define.viewFeedYu);
                UIManager.getInstance().sendMessage(Define.viewPasture,"feed",0);//临时假设1
            }
        }else if(tag=="btn_video"){
            SoundManager.pauseBackGroundSound(true,Common.curMusicRes);
            WXHelper.showVideo(function(state){
                if(state==1){
                    UIManager.getInstance().sendMessage(Define.viewPasture,"feed_double",0);//临时假设1
                }else{//广告没看完
