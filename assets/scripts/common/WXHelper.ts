...
export default class WXHelper {
    ...
    initWXHelper() {
        ...
        if (cc.sys.platform == cc.sys.WECHAT_GAME) {
            ...
            let info = ks.getLaunchOptionsSync()
    ...
    initOnShareAppMessage() {
        ...
        cc.loader.loadRes(shareRes, function (err, data) {
            ks.onShareAppMessage(function (res) {
                return {
                    title: showStr,
    ...
    _updaetSubDomainCanvas() {
        ...
        if (cc.sys.platform == cc.sys.WECHAT_GAME) {
            let openDataContext = window.ks.getOpenDataContext();
            ...
            ks.getSetting({
                success: function (res) {
                    ...
                    if (authSetting['scope.userInfo'] === true) {
                        ...
                        // 用户已拒绝授权，再调用相关 API 或者 ks.authorize 会失败，需要引导用户到设置页面打开授权开关
                        console.log('用户已拒绝授权，再调用相关 API');
                        WXHelper.createWXLoginBtn();
                    } else {
                        // 未询问过用户授权，调用相关 API 或者 ks.authorize 会弹窗询问用户
    ...
    public static createWXLoginBtn() {
        if (cc.sys.platform == cc.sys.WECHAT_GAME) {
            ...
            WXHelper.wxloginBtn = ks.createUserInfoButton({
                type: 'text',
                text: '',
                style: {
                    left: 0,
        ...
        ks.login({
            success: function (res) {
                if (res.code) {
                    console.log("success ..........")
                    console.log(res)
                    //jscode用于后台解密隐私数据encryptedData，参考https://developers.weixin.qq.com/minigame/dev/document/open-api/login/ks.login.html
                    let jscode = res.code;
    ...
    private static bannerAdId = "2300024939_03" //commonBanner
    ...
    public static showBannder(isShow) {
        ...
        let screenHeight = ks.getSystemInfoSync().screenHeight;
        let screenWidth = ks.getSystemInfoSync().screenWidth;
        ...
        if (isShow) {
            ...
            if (WXHelper.bannerAd == null) {
                WXHelper.bannerAd = ks.createBannerAd({
                ...
                WXHelper.bannerAd.onLoad(() => {
                    console.log(`banner 广告加载成功${screenWidth},${screenHeight}`)
    ...
    // 激励式视频
    public static isLoadVideoSuccessful: boolean = true  //广告加载是否成功 会出现加载失败的情况
    public static video = null;
    private static videoId = "2300024939_01"
    ...
    public static initRewardedVideoAd() {
        ...
        WXHelper.video = ks.createRewardedVideoAd({ adUnitId: WXHelper.videoId })

        WXHelper.video.onError(function (errMsg) {
            console.log("广告播放失败-激励广告")
            WXHelper.videoCallBack(2)
        });
        if (WXHelper.video.onLoad == "") {
            WXHelper.isLoadVideoSuccessful = true
        } else {
            WXHelper.video.onLoad(() => {
                console.log("广告加载成功WXHelper.isLoadVideoSuccessful")
                WXHelper.isLoadVideoSuccessful = true
            })
        }
    ...
    static addTotalEvent(eventName: string, eventInfo: any) {//阿拉丁发送事件
        ...
        ks.aldSendEvent(eventName + eventInfo)
    }
